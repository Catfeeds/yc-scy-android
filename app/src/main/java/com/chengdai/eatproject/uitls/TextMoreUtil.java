...
public class TextMoreUtil {
    ...
    public void setShowData(TextView textView, String oriMsg) {
        ...
        if(!TextUtils.isEmpty(oriMsg) && oriMsg.length()>150){
            mTextView.setText(part());
        }else{
            mTextView.setText(oriMsg);
        }

    }

    public TextMoreUtil setSpanTextColor(int spanTextColor){
        mSpanTextColor = spanTextColor;
        return this;
    }

    public TextMoreUtil setOnSpanTextClickListener(OnSpanTextClickListener spanTextClickListener){
        mSpanTextClickListener = spanTextClickListener;
        return this;

    }
    public TextMoreUtil setPos(int startPos, int endPos){
        mStartPos = startPos;
        mEndPos = endPos;
        return this;
    }
   ...
   /*         if (spread) {//调用展开的方法
    ...
    private SpannableString part() {
        String    temp = mOriMsg.substring(0,mOriMsg.length()/2);
        return getSpannableString(temp+"....");/*[更多]*/
    }
    ...
    private SpannableString getSpannableString(CharSequence temp) {
        ...
        spanableInfo.setSpan(new  SpanTextClickable(), mStartPos==0?temp.length()-6:mStartPos, mEndPos==0?temp.length():mEndPos,
